...
import { NamedError } from "@opendoc-ai/util/error"
...
export namespace SessionPrompt {
  ...
  export const PromptInput = z.object({
    ...
    permission: PermissionNext.Ruleset.optional().describe(
      "Permission rules to set on this session. Replaces any existing permissions.",
    ),
  ...
  export const prompt = fn(PromptInput, async (input) => {
    ...
    // Update session permissions if provided
    if (input.permission !== undefined) {
      log.info("permission_update_received", {
        sessionID: input.sessionID,
        ruleCount: input.permission.length,
      ...
      log.info("permission_update_persisted", { sessionID: input.sessionID })

      // Enumerate allowed files based on "read" permission patterns
      const allowedPatterns = input.permission
        .filter((r) => r.action === "allow" && ["read", "glob", "*"].includes(r.permission))
        .map((r) => r.pattern)

      if (allowedPatterns.length > 0) {
        log.info("allowed_patterns", { sessionID: input.sessionID, patterns: allowedPatterns }) 
      }
    }
    // Backwards compatibility for deprecated `tools` field
    else if (input.tools && Object.keys(input.tools).length > 0) {
      ...
      log.info("permission_update_from_tools", {
        sessionID: input.sessionID,
        ruleCount: permissions.length,
      })
    } else {
      log.debug("permission_no_update", {
        sessionID: input.sessionID,
        existingRuleCount: session.permission?.length ?? 0,
      })
    }
  ...
  async function createUserMessage(input: PromptInput) {
    ...
    const agent = await Agent.get(requestedAgentName)
    if (!agent) {
  ...
  async function insertReminders(input: { messages: MessageV2.WithParts[]; agent: Agent.Info; session: Session.Info }) {
    ...
    if (!Flag.OPENDOC_EXPERIMENTAL_PLAN_MODE) {
